...
void setup() {
  ...
  Serial.println("--- Modo de Calibração do Prato ---");
  ...
  Serial.print("Conectando ao servidor de calibração...");
  if (!client.connect(server, port)) { Serial.println(" Falha."); while(1); }
  Serial.println(" Conectado!");
  
  delay(1000);

  Serial.println("A fazer uma rotação de 360 graus para medir o prato...");
  ...
  Serial.println("\n--- Scan do prato concluído. ---");
  client.println("END");
  client.stop();
  motorPrato.disableOutputs();
  Serial.println("Pode fechar o script Python e analisar os resultados.");
  while(1);
}
